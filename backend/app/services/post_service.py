...
from app.schemas.schemas import (CommentCreate, CommentOut, PaginatedPosts,
                                  PostAuthor, PostCreate, PostOut)


def _po(post: Post, uid: int) -> PostOut:
    return PostOut(id=post.id, content=post.content, created_at=post.created_at,
...
def _co(c: Comment) -> CommentOut:
    return CommentOut(id=c.id, content=c.content, created_at=c.created_at,
                      post_id=c.post_id,
                      author=PostAuthor(id=c.author.id, name=c.author.name,
                                        avatar_url=c.author.avatar_url))


def get_feed(db, uid, page=1, per_page=15) -> PaginatedPosts:
    ...
    return PaginatedPosts(posts=[_po(p, uid) for p in posts], total=total,
                          page=page, per_page=per_page, has_more=(offset + per_page) < total)


def create_post(db, data: PostCreate, uid: int) -> PostOut:
    p = Post(user_id=uid, content=data.content)
    db.add(p); db.commit(); db.refresh(p)
    return _po(p, uid)


def delete_post(db, post_id: int, uid: int):
...
def toggle_like(db, post_id: int, uid: int) -> dict:
    p = db.query(Post).filter(Post.id == post_id).first()
    if not p: raise HTTPException(404, "Post not found")
    ...
    if ex:
        ...
        if p.user_id != uid:
            ...
            db.add(Notification(recipient_id=p.user_id, actor_name=a.name,
                                action="liked your post", post_id=post_id)); db.commit()
    db.refresh(p)
    return {"liked": liked, "like_count": len(p.likes)}


def get_comments(db, post_id: int) -> List[CommentOut]:
...
def add_comment(db, post_id: int, data: CommentCreate, uid: int) -> CommentOut:
    p = db.query(Post).filter(Post.id == post_id).first()
    if not p: raise HTTPException(404, "Post not found")
    ...
    if p.user_id != uid:
        a = db.query(User).filter(User.id == uid).first()
        db.add(Notification(recipient_id=p.user_id, actor_name=a.name,
                            action="commented on your post", post_id=post_id)); db.commit()
    return _co(c)


def search_posts(db, q: str, uid: int) -> List[PostOut]:
    return [_po(p, uid) for p in db.query(Post).filter(Post.content.ilike(f"%{q}%"))
